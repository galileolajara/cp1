...
cvar_attr ::= SPACE_AT_REAL_NAME_STR(e).
   { _Mglc_Pcvar_attr_real_name_1(e.basic.id); }
...
decl_include ::= INCLUDE(e).
   { _Mglc_Pdecl_include_begin_3(e.basic.id, e.basic.row, e.basic.col); }
...
at_graves ::= at_graves_count(e).
   { _Mglc_Pat_graves_3(e.basic.id, e.basic.row, e.basic.col); }
at_names ::= at_alias.
...
at(l) ::= type_basic_id(r).
   { l.basic.id = _Mglc_Pat_basic_type_1(r.basic.id); }
at(l) ::= at_names.
...
lvar_decl(l) ::= lvar_decl_name(r).
   { _Mglc_Pdecl_var_end_0(); l.basic.row = r.basic.row; l.basic.col = r.basic.col; }
farg ::= lvar_decl(e) typeAndInfo_optional.
   { _Mglc_Pdecl_var_as_farg_2(e.basic.row, e.basic.col); }
...
expr_type_apply ::= at(r).
   { _Mglc_Pdecl_var_type_1(r.basic.id); }
farg_list ::= farg.
...
func_attr ::= SPACE_AT_REAL_NAME_STR(e).
   { _Mglc_Pfunc_attr_real_name_1(e.basic.id); }
func_attr ::= SPACE_AT_REAL_NAME.
   { _Mglc_Pfunc_attr_real_name_1(-1); }
func_attrs(l) ::= RCBRACE(r).
   { l.basic.row = r.basic.row; l.basic.col = r.basic.col; }
func_attrs(l) ::= func_attr RCBRACE(r).
   { l.basic.row = r.basic.row; l.basic.col = r.basic.col; }
func_attrs_base ::= func_attr.
func_attrs_base ::= func_attrs_base func_attr.
func_attrs(l) ::= func_attrs_base func_attr RCBRACE(r).
   { l.basic.row = r.basic.row; l.basic.col = r.basic.col; }
...
// expr_type(l) ::= at(r). { l.basic.id = _Mglc_Pexpr_type_1(r.basic.id); }
func_type ::= typeAndInfo_optional.
func_decl ::= func_decl_begin fargs func_type func_attrs(end).
   { _Mglc_Pdecl_func_end_2(end.basic.row, end.basic.col); }
...
typeInfo_ref ::= ref_count(c).
   { _Mglc_Ptype_info_ref_1(c.basic.id); }
...
typeInfo_arr_exprs ::= lbracket_or_space typeInfo_arr_exprs1(e) rbracket_or_comma.
   { _Mglc_Ptype_info_arr_2(e.index.v, e.index.c); }
...
expr_int(l) ::= NUM_DEC(e).
   { l.basic.id = _Mglc_Pexpr_int_2(e.basic.id, 0); }
expr_int(l) ::= NUM_HEX(e).
   { l.basic.id = _Mglc_Pexpr_int_2(e.basic.id, 1); }
expr_int(l) ::= NUM_OCT(e).
   { l.basic.id = _Mglc_Pexpr_int_2(e.basic.id, 2); }

expr_str(l) ::= STRING.
   { l.basic.id = _Mglc_Pexpr_str_1(-1); }
expr_str(l) ::= expr_str(e) SPACE STRING.
   { l.basic.id = _Mglc_Pexpr_str_1(e.basic.id); }
...
expr_lvar_plus(l) ::= expr_lvar_plus(r) PLUS.
   { l.basic.id = r.basic.id + 1; }
expr_lvar(e) ::= ID(var).
   { e.basic.id = _Mglc_Pexpr_lvar_4(var.basic.id, 0, var.basic.row, var.basic.col); }
expr_lvar(e) ::= expr_lvar_plus(plus) ID(var) expr_type_none.
   { e.basic.id = _Mglc_Pexpr_lvar_4(var.basic.id, plus.basic.id, var.basic.row, var.basic.col); }
expr_lvar(e) ::= expr_lvar_plus(plus) lparen_or_space ID(var) typeAndInfo rparen_or_space.
   { e.basic.id = _Mglc_Pexpr_lvar_4(var.basic.id, plus.basic.id, var.basic.row, var.basic.col); }
expr_cvar(l) ::= ID_HASH(e).
   { l.basic.id = _Mglc_Pexpr_cvar_2(-1, e.basic.id); }
expr_cvar(l) ::= at(at) ID_HASH(e).
   { l.basic.id = _Mglc_Pexpr_cvar_2(at.basic.id, e.basic.id); }
value(l) ::= value4fix(r).
...
value4arr(l) ::= nullExpr(r).
   { l.basic.id = r.basic.id; }
charExpr(l) ::= CHAR1(c).
   { l.basic.id = _Mglc_Pexpr_char_1(c.basic.id); }
charExpr(l) ::= CHAR2(c).
   { l.basic.id = _Mglc_Pexpr_char_1(c.basic.id); }
boolExpr(l) ::= GRAVE_TRUE.
   { l.basic.id = _Mglc_Pexpr_bool_1(1); }
boolExpr(l) ::= GRAVE_FALSE.
   { l.basic.id = _Mglc_Pexpr_bool_1(0); }
nullExpr(l) ::= GRAVE_NULL.
   { l.basic.id = _Mglc_Pexpr_null_0(); }
...
expr_and1(l) ::= expr_and1(left) COMMA_SPACE expr(right).
   { l.basic.id = _Mglc_Pexpr_bools_add_2(left.basic.id, right.basic.id); }
expr_and(l) ::= LPAREN_AMP_AMP_COMMA_SPACE expr_and1(r) rparen_or_comma.
   { l.basic.id = r.basic.id; }
expr_or1(l) ::= expr(left) COMMA_SPACE expr(right).
   { l.basic.id = _Mglc_Pexpr_bools_3(left.basic.id, right.basic.id, 1); }
...
compareExpr(l) ::= value(a) compare_type(t) value(b).
   { l.basic.id = _Mglc_Pexpr_compare_3(a.basic.id, b.basic.id, t.basic.id); }
...
call_arg ::= expr(e).
   { _Mglc_Pcarg_push_1(e.basic.id); }
call_arg ::= expr_str(e) HASH.
   { _Mglc_Pcarg_push_str_1(e.basic.id); }
...
callExpr_func(l) ::= funcExpr(e) call_args_optional.
   { l.basic.id = _Mglc_Pexpr_pop_func_2(e.basic.id, e.basic.id2); }
callExpr_method(l) ::= methodExpr(e) call_args_optional.
   { l.basic.id = _Mglc_Pexpr_pop_method_2(e.basic.id, e.basic.id2); }
...
assignExpr1(l) ::= value(left) assign_type1(type) expr(right).
   { l.basic.id = _Mglc_Pexpr_assign_3(left.basic.id, right.basic.id, type.basic.id); }
...
expr2stmt ::= begin_pos(begin) expr2stmt_base(e) end_pos(end).
   { _Mglc_Pexpr2stmt_5(e.basic.id, begin.basic.row, begin.basic.col, end.basic.row, end.basic.col); }
...
sizeOfTypeExpr(l) ::= at(at) LBRACKET GRAVE_SIZE RBRACKET.
   { l.basic.id = _Mglc_Pexpr_size_of_type_1(at.basic.id); }

fastCastExpr(l) ::= value4cast(e) at(at).
   { l.basic.id = _Mglc_Pexpr_cast_fast_2(e.basic.id, at.basic.id); }

negVal(l) ::= MINUS value4fix(e).
   { l.basic.id = _Mglc_Pexpr_unary_2(e.basic.id, 0); }

incExpr(l) ::= value(e) PLUS PLUS.
   { l.basic.id = _Mglc_Pexpr_unary_2(e.basic.id, 1); }

decExpr(l) ::= value(e) MINUS MINUS.
   { l.basic.id = _Mglc_Pexpr_unary_2(e.basic.id, 2); }

refExpr(l) ::= AMPERSAND SPACE value(e).
   { l.basic.id = _Mglc_Pexpr_ref_1(e.basic.id); }
/*
  expr
  { $$.v[0] = $expr; $$.c = 1; }
...
indexExpr(l) ::= valueonly(r) lbracket_or_space indexExpr_exprs(e) rbracket_or_comma.
   { l.basic.id = _Mglc_Pexpr_index_3(r.basic.id, e.index.v, e.index.c); }
...
do_expr ::= begin_pos(begin) do_expr_val(e) end_pos(end).
   { _Mglc_Pstmt_do_set_6(e.basic.id, begin.basic.row, begin.basic.col, end.basic.row, end.basic.col, 0); }
do_continue_begin(l) ::= .
...
while_expr ::= begin_pos(begin) while_expr_val(e) end_pos(end).
   { _Mglc_Pstmt_while_set_6(e.basic.id, begin.basic.row, begin.basic.col, end.basic.row, end.basic.col, 0); }
...
stmt_continue ::= begin_pos(begin) LCBRACE_CONTINUE rcbrace_or_space end_pos(end).
   { _Mglc_Pstmt_continue_5(-1, begin.basic.row, begin.basic.col, end.basic.row, end.basic.col); }
...
stmt_lvars ::= decl_lvar_begin lvar_list(end).
   { _Mglc_Pstmt_lvar_end_2(end.basic.row, end.basic.col); }
decl_lvar ::= ID(var) typeAndInfo_optional SPACE_EQUAL_SPACE expr(e).
   { _Mglc_Pstmt_lvar_add_4(var.basic.id, e.basic.id, var.basic.row, var.basic.col); }
decl_lvar ::= ID(var) typeAndInfo_optional.
   { _Mglc_Pstmt_lvar_add_4(var.basic.id, -1, var.basic.row, var.basic.col); }
...
decl_var_attr ::= SPACE_AT_REAL_NAME_STR(e).
   { _Mglc_Pdecl_var_attr_real_name_1(e.basic.id); }
...
stmts_optional ::= rcbrace_or_space.
...
decl_struct_attr ::= SPACE_AT_REAL_NAME_STR(e).
   { _Mglc_Pstruct_attr_real_name_1(e.basic.id); }
...
decl_struct_attrs_optional ::= decl_struct_attrs_list end_pos(end).
   { _Mglc_Pdecl_struct_end_2(end.basic.row, end.basic.col); }
decl_func ::= LCBRACE func_decl stmts_optional.
